...
\begin{frame}[plain]{}
    \begin{center}
        ...
        Test this image processing node
    \end{center}
\end{frame}
...
\begin{frame}[plain,fragile]{}
    \begin{center}
        \Large
        OfficeBots (with plenty of Wall-Es!)

        \begin{center}
            \includegraphics[width=0.5\linewidth]{OfficeBots.png}
        \end{center}

        \begin{enumerate}
            \item \sh{pip3 install officebots}
            \item go to \url{research.skadge.org}
            \item start the ROS bridge:
\begin{shcode}
> roscore
# in a different terminal
> officebots-ros <name of your robot>
\end{shcode}
            \item click the blue 'sleepy robot' icon in the game
...
\begin{frame}[plain]{}
    \begin{itemize}
        \item what are the available topics? (check \sh{rostopic
            list|info|echo})
        \item what are the topic we can write on? read from?
        \item what data type is exchanged?
    \end{itemize}
\end{frame}
...
\begin{frame}[plain]{}
    \begin{center}
        ...
        \begin{itemize}
            ...
            \item in a graphical way?
            \item \small{(check \sh{rviz}!)}
            \item Write a small ROS node to control your robot from the
                keyboard
                (\href{https://github.com/severin-lemaignan/robomaze/blob/ros/scripts/keyboard.py}{use
...
\begin{frame}{2D navigation: what do we already have?}
    ...
    \begin{itemize}
        ...
        \item<+-> a controller (publishing to the \texttt{/cmd\_vel} topic)
    \end{itemize}
...
\begin{shcode}
> sudo apt install ros-noetic-move-base ros-noetic-slam-gmapping ros-noetic-map-server
\end{shcode}

Launch your TF broadcaster, and then:

\begin{shcode}
> rosrun gmapping slam_mapping _map_update_interval:=1 _linearUpdate:=0.2 _angularUpdate:=0.2
\end{shcode}
...
\begin{frame}{Mapping (2)}
    ...
    You can now stop \texttt{gmapping}.
